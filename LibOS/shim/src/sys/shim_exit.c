...
unsigned long usages[0x10] = { 0 };
unsigned long sums[0x10] = { 0 };

static int print_done = 0;
static unsigned long _usages[0x10] = { 0 };
static unsigned long _sums[0x10] = { 0 };
...
if (__atomic_compare_exchange_n(&print_done, &x, 1, 0, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
    size_t i;
    for (i = 0; i < 0x10; ++i) {
        _sums[i] = __atomic_load_n(&sums[i], __ATOMIC_RELAXED);
        _usages[i] = __atomic_load_n(&usages[i], __ATOMIC_RELAXED);
    }
    ...
    for (i = 0; i < 0x10; ++i) {
        if (_usages[i]) {
            snprintf(buf, sizeof buf, "%2lu: %10lu %10lu %10lu\n", i, _sums[i], _usages[i], _sums[i] / _usages[i]);
            shim_do_write(1, buf, strlen(buf));
        }
    }
}
